...
hands = mp_hands.Hands(
    ...
    max_num_hands=2,             # Track both hands for two-palm clear
    min_detection_confidence=0.5,   # Lower = detects hands more eagerly
...
PALETTE_H = 80          # Height of the palette bar at top
SWATCH_W = 100          # Width of each color swatch
...
hover_index = -1
hover_frames = 0
HOVER_THRESHOLD = 20    # Frames finger must hover to select

# --- Two-palm clear state ---
clear_frames = 0
CLEAR_THRESHOLD = 30    # Frames both palms must be held to clear
...
def is_clear_gesture(fingers, lm):
    """All 5 fingers must be clearly extended — stricter check"""
    all_up = all(fingers)
    index_clear  = lm[8].y  < lm[5].y
    middle_clear = lm[12].y < lm[9].y
    ring_clear   = lm[16].y < lm[13].y
    pinky_clear  = lm[20].y < lm[17].y
    return all_up and index_clear and middle_clear and ring_clear and pinky_clear

def draw_palette(frame, selected_index, hover_index, hover_frames):
    """Draw the color palette bar onto the frame."""
...
def get_swatch_at(cx, cy):
    """Return swatch index if finger is in the palette zone, else -1."""
    if cy < PALETTE_H:
        i = cx // SWATCH_W
        if 0 <= i < len(palette):
            return i
...
while True:
    ...
    if result.multi_hand_landmarks:
        ...
        # --- Check if ALL detected hands are open palms ---
        ...
        if num_hands == 2:
            ...
            for hand_landmarks in result.multi_hand_landmarks:
                ...
                palm_checks.append(is_clear_gesture(f, lm))
    ...
    # --- Draw palette on top of everything ---
